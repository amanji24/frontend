...
class Tables extends React.Component {
  ...
  changePoints = (event) => {
    event.preventDefault();
  //  console.log(this.state.usernewpoints)
    const parsed = parseInt(this.state.newpoints)
    this.changeUserPoints(parsed,this.state.usernewpoints)
  }
  ...
  render() {
   // console.log('tag :', this.state.tag)
    ...
    let table_rows = allPosts.map((post) => {
      ...
      if (post.tag !== this.state.filterTag && this.state.filterTag !== 'All') return
      // let display;
      // if (adminTrue) {
      //   display = (
              ...
              <Card className="shadow">
                <CardHeader style={{ backgroundRepeat: 'repeat', backgroundImage: 'url("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSNarfhvpyI9_wzGSXbXUH5Yn0Q9CAApXoBz8sDxKapfADJkqbiDw")' }} className="border-0">
                  <Row>
                    ...
                    <select className="custom-select col-2" style={{color:'black'}} onChange={(o) => this.setState({ filterTag: o.target.value })}>
                      {tags.map(o => <option style={{color:'black'}} value={o} selected={this.state.tag == o}>{o}</option>)}
                    </select>
                    ...
                    <Modal isOpen={this.state.modal} toggle={this.toggle} className={this.props.className}>
                      ...
                      <ModalBody>
                        <Form role="form" onSubmit={this.onSubmit}>
                          ...
                          <ModalFooter>
                            <select className="custom-select col-3" style={{color:'black'}} onChange={(o) => this.setState({ tag: o.target.value })} value={this.state.tag}>
                              {tags.map(o => <option style={{color:'black'}}value={o} selected={this.state.tag == o}>{o}</option>)}
                            </select>
